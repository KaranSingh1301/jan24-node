...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    //genrate the token
    const verifyToken = genrateJWTToken(email);

    //send email
    sendVerificationEmail({ email, verifyToken });

    return res.send(
      "link has been sent to you mail, please verify before login"
    );
  } catch (error) {
    console.log(error);
    return res.send({
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (userDb && !userDb.isEmailAuthenticated) {
      return res.send({
        status: 400,
        message: "Please authenticate your email before login.",
      });
    }
